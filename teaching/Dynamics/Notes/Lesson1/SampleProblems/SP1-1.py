...
d=36
tfinal=6
t = np.linspace(0, tfinal, 1000, endpoint=True)
...
textdata=fig.text(0.02, 0.84, "", bbox=props, fontname='Courier New')

    
# Top (unshared) axes
topax = fig.add_subplot(gs_top[0,:])
topax.set_xlim(-50, 50)
topax.set_ylim(-5, 5)
topax.get_yaxis().set_ticks([])
topax.set_xlabel('Position (m)') 
...
bottom_axes[0].axhline(y=0, color='gray', linestyle='dotted', linewidth=1)
bottom_axes[0].margins(0.05)
bottom_axes[0].set_ylabel('$x$ (m)',rotation='horizontal', ha='right')
bottom_axes[0].set_ylim(-60, 60)
bottom_axes[0].set_yticks([-30, 0, 30])
...
bottom_axes[1].axhline(y=0, color='gray', linestyle='dotted', linewidth=1)
bottom_axes[1].margins(0.05)
bottom_axes[1].set_ylabel('$v$ (m/s)',rotation='horizontal', ha='right') 
bottom_axes[1].set_ylim(-60, 60)
bottom_axes[1].set_yticks([-30, 0, 30])
...
bottom_axes[2].axhline(y=0, color='gray', linestyle='dotted', linewidth=1)
bottom_axes[2].margins(0.15)
bottom_axes[2].set_xlabel('time (s)')
bottom_axes[2].set_ylabel('$a$ (m/s$^2$) ',rotation='horizontal', ha='right') 
bottom_axes[2].set_ylim(-15, 40)
bottom_axes[2].set_yticks([0,15,30])
...
def update(myt):
    ...
    datastring = '\n'.join((
    r'$t=\mathtt{%.2f}~\text{s}$' % (myt, ),
    r'$x=\mathtt{%.2f}~\text{m}$' % (pos(myt), ),
    r'$v=\mathtt{%.2f}~\text{m/s}$' % (vel(myt), ),
    r'$a=\mathtt{%.2f}~\text{m/s}^2$' % (acc(myt), )
    ))
    textdata.set_text(datastring)

    return toppoint, pospoint, velpoint, accpoint, posline, velline, accline
   

ani = animation.FuncAnimation(fig, func=update, blit=False, interval=1000./30., frames=np.linspace(0,tfinal,30*tfinal,endpoint=True))
Writer = animation.writers['ffmpeg']
writer = Writer(fps=30, metadata=dict(artist='Kevin Dusling'), bitrate=1800)

# 6. Save the animation
ani.save('SP1-1.mp4', writer=writer)
